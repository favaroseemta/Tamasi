...
export default {
    data: new SlashCommandBuilder()
        .setName("ping")
        .setDescription("Ellenorzi a bot keslelteteset es az API sebesseget"),

    async prefixExecute(interaction) {
        try {
            const startTime = Date.now();
            const pingingMessage = await interaction.reply({ content: 'Meres...' });
            ...
            const embed = createEmbed({ title: 'Pong!', description: null }).addFields(
                { name: 'Bot kesleltetes', value: `${latency}ms`, inline: true },
                { name: 'API kesleltetes', value: `${apiLatency}ms`, inline: true },
            );
            ...
            if (!interaction.replied && !interaction._replyMessage) {
                await interaction.channel.send({
                    embeds: [createEmbed({ title: 'Rendszerhiba', description: 'Jelenleg nem sikerult meghatarozni a kesleltetest.', color: 'error' })],
                }).catch(() => {});
            }
        }
    },
